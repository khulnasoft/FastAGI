...
@patch('startagi.controllers.analytics.db')
def test_get_metrics_success(mock_get_db):
    with patch('startagi.helper.auth.get_user_organisation') as mock_get_user_org, \
                patch('startagi.controllers.analytics.db') as mock_db, \
                patch('startagi.controllers.analytics.AnalyticsHelper') as mock_helper, \
...
def test_get_tools_user_success(mock_get_db):
    with patch('startagi.helper.auth.get_user_organisation') as mock_get_user_org, \
                    patch('startagi.controllers.analytics.db') as mock_db, \
                    patch('startagi.controllers.analytics.ToolsHandler') as mock_handler, \
                    patch('startagi.helper.auth.db') as mock_auth_db:
