...
@patch('fastagi.controllers.models_controller.db')
...
@patch('fastagi.controllers.models_controller.db')
@patch('fastagi.controllers.models_controller.ModelsConfig.fetch_api_key', return_value = {})
...
def test_get_marketplace_models_list_success(mock_get_db):
    with patch('fastagi.helper.auth.get_user_organisation') as mock_get_user_org, \
        patch('fastagi.helper.auth.db') as mock_auth_db, \
        patch('fastagi.controllers.models_controller.requests.get') as mock_get:
