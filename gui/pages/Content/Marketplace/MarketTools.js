...
export default function MarketTools() {
  ...
  useEffect(() => {
    ...
    if (window.location.href.toLowerCase().includes('marketplace')) {
      ...
      axios.get('https://app.startagi.khulnasoft.com/api/toolkits/marketplace/list/0')
        .then((response) => {
            ...
            <div className="market_containers cursor_pointer" key={item.id} onClick={() => handleTemplateClick(item)}>
              <div className="horizontal_container overflow_auto">
                ...
                <div className="ml_12 mb_8">
                    <div>{item.name}</div>
                    <div className="color_gray lh_16">by StartAgi&nbsp;<Image width={14} height={14} src="/images/is_verified.svg" alt="is_verified"/></div>
                </div>
              </div>
