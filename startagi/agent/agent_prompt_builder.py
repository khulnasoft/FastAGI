...
class AgentPromptBuilder:
    ...
    @classmethod
    def replace_main_variables(cls, fast_agi_prompt: str, goals: List[str], instructions: List[str], constraints: List[str],
                               tools: List[BaseTool], add_finish_tool: bool = True):
        """Replace the main variables in the fast agi prompt.

        Args:
            fast_agi_prompt (str): The fast agi prompt.
        ...
        """
        fast_agi_prompt = fast_agi_prompt.replace("{goals}", AgentPromptBuilder.add_list_items_to_string(goals))
        if len(instructions) > 0 and len(instructions[0]) > 0:
            ...
            fast_agi_prompt = fast_agi_prompt.replace("{instructions}", "INSTRUCTION: " + '\n' +  AgentPromptBuilder.add_list_items_to_string(instructions))
            fast_agi_prompt = fast_agi_prompt.replace("{task_instructions}", task_str + '\n' +  AgentPromptBuilder.add_list_items_to_string(instructions))
        else:
            fast_agi_prompt = fast_agi_prompt.replace("{instructions}", '')
        fast_agi_prompt = fast_agi_prompt.replace("{task_instructions}", "")
        fast_agi_prompt = fast_agi_prompt.replace("{constraints}",
        ...
        fast_agi_prompt = fast_agi_prompt.replace("{tools}", tools_string)
        return fast_agi_prompt

    @classmethod
    def replace_task_based_variables(cls, fast_agi_prompt: str, current_task: str, last_task: str,
                                     last_task_result: str, pending_tasks: List[str], completed_tasks: list, token_limit: int):
        """Replace the task based variables in the fast agi prompt.
        ...
        """
        if "{current_task}" in fast_agi_prompt:
            fast_agi_prompt = fast_agi_prompt.replace("{current_task}", current_task)
        if "{last_task}" in fast_agi_prompt:
            fast_agi_prompt = fast_agi_prompt.replace("{last_task}", last_task)
        if "{last_task_result}" in fast_agi_prompt:
            fast_agi_prompt = fast_agi_prompt.replace("{last_task_result}", last_task_result)
        if "{pending_tasks}" in fast_agi_prompt:
            fast_agi_prompt = fast_agi_prompt.replace("{pending_tasks}", str(pending_tasks))

        completed_tasks.reverse()
        if "{completed_tasks}" in fast_agi_prompt:
            completed_tasks_arr = []
            for task in completed_tasks:
                completed_tasks_arr.append(task['task'])
            fast_agi_prompt = fast_agi_prompt.replace("{completed_tasks}", str(completed_tasks_arr))

        base_token_limit = TokenCounter.count_message_tokens([{"role": "user", "content": fast_agi_prompt}])
        pending_tokens = token_limit - base_token_limit
        final_output = ""
        if "{task_history}" in fast_agi_prompt:
            for task in reversed(completed_tasks[-10:]):
                ...
                if token_count > min(600, pending_tokens):
                    break
            fast_agi_prompt = fast_agi_prompt.replace("{task_history}", "\n" + final_output + "\n")
        return fast_agi_prompt
